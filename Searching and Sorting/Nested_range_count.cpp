...
int main() {
    ...
    contained[arr[0]] = 0;
    multiset<ll>small, great;
    great.insert(arr[0].second);
    //we want gerater value
    for (int i = 1; i < n; i++)
    {
        auto it = great.lower_bound(arr[i].second);
        if (it == great.end())
        {
            continue;
        }
        ...
        {
            contained[arr[i]] = distance(it, great.end());
        }
        // maxi = max(arr[i].second, maxi);
        great.insert(arr[i].second);
    }
...
//container logic
    ...
    container[arr[n - 1]] = 0;
    small.insert(-arr[n - 1].second);
    for (int i = n - 2; i > -1; i--)
    {
        auto it = small.lower_bound(-arr[i].second);
        if (it != great.end())
        {
            container[arr[i]] = distance(it, small.end());
        }
        small.insert(-arr[i].second);
    }
